...
def recuperar_contrasena(request):
    if request.method == 'POST':
        ...
        try:
            usuario = Usuario.objects.get(correo=correo, pregunta_recuperacion=pregunta)
            if usuario.respuesta_pregunta.lower() == respuesta.lower():
                # Guardar el ID del usuario en la sesión
                request.session['usuario_id'] = usuario.id
                
                # Redirigir a la página de restablecer contraseña
                messages.success(request, "Respuesta correcta. Ahora puedes restablecer tu contraseña.")
                return redirect('restablecer_contrasena')  # Usa el nombre correcto aquí
            else:
                messages.error(request, "Respuesta incorrecta. Inténtalo nuevamente.")
        except Usuario.DoesNotExist:
            messages.error(request, "No se encontró un usuario con ese correo y pregunta.")
    
    preguntas = Usuario.PREGUNTAS_RECUPERACION
    return render(request, 'core/recuperar.html', {'preguntas': preguntas})
...
def restablecer_contrasena(request):
    if request.method == 'POST':
        ...
        # Recuperar al usuario desde la sesión
        usuario_id = request.session.get('usuario_id')

        if not usuario_id:
            messages.error(request, "No se pudo verificar tu identidad. Inténtalo nuevamente.")
            return redirect('recuperar')  # Cambia al nombre de la vista
        ...
        try:
            ...
            return redirect('login')  # Cambia al nombre de la vista de inicio de sesión
        except Usuario.DoesNotExist:
